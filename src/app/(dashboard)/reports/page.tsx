...
import { useSecurityStore, SecurityScan } from '@/store/securityStore';
...
export default function ReportsPage() {
  const recentScans = usePersistedStore(
    useSecurityStore,
  ...
  const { clearScans } = useSecurityStore();

  // Helper: map scan type to icon
  const getScanIcon = (type: string) => {
    switch (type) {
      case 'password': return <Key className="w-4 h-4 text-[#0A0A0A]" />;
      case 'email': return <Mail className="w-4 h-4 text-[#0A0A0A]" />;
      case 'network': return <Globe className="w-4 h-4 text-[#0A0A0A]" />;
      case 'phishing': return <Search className="w-4 h-4 text-[#0A0A0A]" />;
      case 'ssl': return <ShieldCheck className="w-4 h-4 text-[#16A34A]" />;
      default: return <FileText className="w-4 h-4 text-[#4B5563]" />;
    }
  };

  // Generate and download audit report file
...
----------------------------------------------------------------------
${monitoredEmails.length === 0 ? 'No monitored email accounts registered.' : 
  monitoredEmails.map((m, idx) => {
    return `${idx + 1}. Email: ${m.email}
   - Verification status: ${m.isVerified ? 'VERIFIED' : 'UNVERIFIED'}
   - Automated alerts: ${m.alertsEnabled ? 'ENABLED' : 'DISABLED'}
...
----------------------------------------------------------------------
${recentScans.length === 0 ? 'No recent security scan activities logged.' :
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-black p-6 rounded-none hover:shadow-[4px_4px_0px_rgba(0,0,0,1)] transition-shadow duration-100">
            ...
            {recentScans.length === 0 ? (
              <div className="py-16 text-center border border-dashed border-black rounded-none bg-[#F8F9FA] font-mono">
                ...
                <p className="text-xs text-[#0A0A0A] font-bold uppercase tracking-tight">No security audit activities recorded yet.</p>
                <p className="text-[11px] text-[#4B5563] mt-1 font-medium">Run password check or port scans to log history items.</p>
              </div>
            ) : (
              <div className="space-y-4 max-h-[600px] overflow-y-auto pr-1">
                {recentScans.map((scan) => (
                  <div key={scan.id} className="bg-white border border-black p-4 rounded-none flex items-start gap-3 hover:shadow-[4px_4px_0px_rgba(0,0,0,1)] transition-shadow duration-100">
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex justify-between items-center">
                        <span className="text-xs font-semibold text-[#4B5563] uppercase tracking-wider font-mono">
                          {scan.type} scan: <code className="font-mono text-[#0A0A0A] font-bold ml-1">{scan.target}</code>
                        </span>
                      ...
                      <div className="flex justify-between items-center pt-2">
                        ...
                        <span className={`text-[9px] font-mono px-2 py-0.5 rounded-none border border-black uppercase font-bold tracking-wider ${
                          scan.status === 'danger' ? 'bg-[#DC2626] text-white' :
                          scan.status === 'warning' ? 'bg-[#EAB308] text-black' :
        ...
        <div className="space-y-6">
          <div className="bg-white border border-black p-6 space-y-4 rounded-none hover:shadow-[4px_4px_0px_rgba(0,0,0,1)] transition-shadow duration-100">
            <h3 className="text-sm font-heading font-bold text-[#0A0A0A] uppercase tracking-tight">&gt; SECURITY SCOREBOARD</h3>

            <div className="space-y-3">
              <div className="p-4 bg-[#F8F9FA] border border-black rounded-none text-center space-y-1 font-mono">
                <span className="text-sm text-[#4B5563] font-semibold block uppercase font-mono">Compliance Grade</span>
                <span className="text-3xl font-extrabold text-[#16A34A] font-heading">A-</span>
                <span className="text-[10px] text-[#4B5563] block font-mono">FULLY COMPLIANT WITH STANDARD RULES</span>
              </div>

              <div className="bg-[#F8F9FA] border border-black p-3.5 rounded-none space-y-2 text-xs font-mono">
                <span className="text-[#4B5563] uppercase text-[10px] font-bold block font-mono">AUDIT STATS:</span>
          ...
          <div className="bg-white border border-black p-6 flex items-start space-x-3 text-xs rounded-none hover:shadow-[4px_4px_0px_rgba(0,0,0,1)] transition-shadow duration-100">
            ...
            <div className="text-[#4B5563] font-mono">
              <strong className="block text-[#0A0A0A] mb-1 uppercase tracking-tight">&gt; WEEKLY DIGEST ALERTS</strong>
